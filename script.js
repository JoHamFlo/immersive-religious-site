...
function renderIslamicNames(){
  ...
  grid.innerHTML = filtered.map(item => `
    <article class="name-card clickable-card ${item.connection ? 'has-connection' : ''}" tabindex="0" role="button" aria-label="Open theology information for ${item.name}" data-name-number="${item.n}">
      ...
      <p class="tap-hint">Click card for deeper theology</p>
    </article>
  `).join('') || '<p class="empty">No names match your search.</p>';
  document.querySelectorAll('#islamicNamesGrid .clickable-card').forEach(card => {
    card.addEventListener('click', () => openIslamicTheologyPanel(card.dataset.nameNumber));
    card.addEventListener('keydown', event => {
      if(event.key === 'Enter' || event.key === ' '){
        event.preventDefault();
        openIslamicTheologyPanel(card.dataset.nameNumber);
      }
    });
  });
}
...
function openIslamicTheologyPanel(number){
...
function buildChristianPanel(item, index){
  const [name, meaning, theology] = item;
  return `
    <p class="modal-eyebrow christian-eyebrow">Unique Christian Name ${index + 1}</p>
    <div class="modal-heading-row">
      <div><h2 id="modalTitle">${name}</h2><p class="modal-meaning christian-meaning">${meaning}</p></div>
    </div>
    <div class="deep-grid">
      <section class="deep-section christian-deep"><h3>Core Christian Theology</h3><p>${theology}</p></section>
      <section class="deep-section christian-deep"><h3>Why This Name Matters</h3><p>This name emphasizes how Christians describe God personally through covenant, rescue, holiness, presence, protection, judgment, healing, or salvation.</p></section>
      <section class="deep-section christian-deep"><h3>Connection to Faith Practice</h3><p>Names like this are often used in prayer, worship, preaching, and reflection because they help believers connect God’s character to real human needs.</p></section>
      <section class="deep-section christian-deep"><h3>Comparative Lens</h3><p>This name can be compared with Islamic names of God by asking whether both traditions emphasize similar divine attributes, while still respecting important theological differences.</p></section>
    </div>
    <div class="reflection-box christian-reflection"><h3>Exhibit Reflection Question</h3><p>What does this name reveal about how Christians understand God’s relationship with humanity?</p></div>
  `;
}

function openChristianTheologyPanel(index){
  const item = christianNames[Number(index)];
  if(!item) return;
  modalContent.innerHTML = buildChristianPanel(item, Number(index));
  modal.classList.add('show');
  modal.setAttribute('aria-hidden', 'false');
  closeModalButton.focus();
}
...
function renderChristianNames(){
  document.querySelector('#christianNamesGrid').innerHTML = christianNames.map(([name,meaning,theology], index) => `
    <article class="name-card christian-card clickable-card" tabindex="0" role="button" aria-label="Open theology information for ${name}" data-christian-index="${index}">
      <h3>${name}</h3>
      <p class="meaning">${meaning}</p>
      <p>${theology}</p>
      <p class="tap-hint christian-hint">Click card for deeper theology</p>
    </article>
  `).join('');
  document.querySelectorAll('#christianNamesGrid .clickable-card').forEach(card => {
    card.addEventListener('click', () => openChristianTheologyPanel(card.dataset.christianIndex));
    card.addEventListener('keydown', event => {
      if(event.key === 'Enter' || event.key === ' '){
        event.preventDefault();
        openChristianTheologyPanel(card.dataset.christianIndex);
      }
    });
  });
}
